...
export async function fetchTransformerData(): Promise<TransformerData> {
  ...
  try {
    ...
    for (let i = 0; i < offsets.length; i += 10) {
      ...
      const results = await Promise.allSettled(
        batch.map((offset) => {
          ...
          return fetch(pageUrl.toString(), { signal: controller.signal });
        })
      );
    ...
    const transformers = allResults
      .filter((r: Record<string, unknown>) => r.coordenadas_geo != null)
